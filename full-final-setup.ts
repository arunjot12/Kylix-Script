...
const ORACLE_ACCOUNT = '5CofGsPhVSLRJbhSG9CDxvNmjMKUs7FYnDUyCUVAmK92rjVU';
...
async function main() {
  ...
  // Step 2: Transfer native balance to oracle account
  console.log(`\n${'='.repeat(60)}`);
  console.log(`💸 Transferring native balance to Oracle Account`);
  console.log(`${'='.repeat(60)}`);
  console.log(`📤 Transferring to ${ORACLE_ACCOUNT}...`);
  const oracleTransferAmount = new BN('500000000000000'); // 0.5 units
  
  await new Promise<void>((resolve, reject) => {
    api.tx.balances
      .transferKeepAlive(ORACLE_ACCOUNT, oracleTransferAmount)
      .signAndSend(admin, { nonce: nonce++ }, ({ status, dispatchError }) => {
        if (dispatchError) {
          if (dispatchError.isModule) {
            const decoded = api.registry.findMetaError(dispatchError.asModule);
            console.error(`❌ Oracle transfer failed: ${decoded.section}.${decoded.name}`);
          } else {
            console.error(`❌ Oracle transfer failed: ${dispatchError.toString()}`);
          }
          reject(dispatchError);
        } else if (status.isInBlock) {
          console.log(`✅ Oracle account funded in block ${status.asInBlock}`);
          resolve();
        }
      });
  });
  ...
  // Step 3: Add oracle member using sudo
  console.log(`\n${'='.repeat(60)}`);
  console.log(`🔮 Adding Oracle Member`);
  console.log(`${'='.repeat(60)}`);
  console.log(`👤 Adding ${ORACLE_ACCOUNT} as oracle member...`);
  
  const addOracleMemberTx = api.tx.oracleMembership.addMember(ORACLE_ACCOUNT);
  const sudoAddOracleTx = api.tx.sudo.sudo(addOracleMemberTx);

  await new Promise<void>((resolve, reject) => {
    sudoAddOracleTx.signAndSend(admin, { nonce: nonce++ }, ({ status, dispatchError }) => {
      if (dispatchError) {
        if (dispatchError.isModule) {
          const decoded = api.registry.findMetaError(dispatchError.asModule);
          console.error(`❌ Add oracle member failed: ${decoded.section}.${decoded.name}`);
        } else {
          console.error(`❌ Add oracle member failed: ${dispatchError.toString()}`);
        }
        reject(dispatchError);
      } else if (status.isInBlock) {
        console.log(`✅ Oracle member added in block ${status.asInBlock}`);
        resolve();
      }
    });
  });
  ...
  // Step 4: Set base asset (USD - asset ID 1)
  console.log(`\n${'='.repeat(60)}`);
